	...
	public function __construct($action, $post = array()) {
		...
		switch ($this->action) {
			...
			case "installationTestDbSettings":
				...
				list($success, $content) = Database::testDbSettings($this->post["dbHostname"], $this->post["dbPort"], $this->post["dbName"], $this->post["dbUsername"], $this->post["dbPassword"]);
			...
			case "installationCreateSettingsFile":
				...
				if (Core::checkSettingsFileExists()) {
					...
					list($success, $content) = Installation::createSettingsFile($this->post["dbHostname"], $this->post["dbPort"],
